...
The contract has two halves. First, exact reduction: at Γ = 0 the tensor
and the 6×6 added-mass matrix must reproduce every planar term
`l1_trim.mass_props` already uses — if they don't, the lateral model is
wrong and everything built on it inherits the error. Second, the two
lateral facts that decide the modes: roll is added-mass dominated, and
sideslip added mass is negligible.
"""
import math
...
def test_planar_terms_reproduce_l1_trim(specs, p3):
    ...
    # 1e-4: same trapezoid rule, but 401 strips here vs l1_trim's 201,
    # and the integrand has a kink at the root from chord_at's |y|
    assert p3.added[2, 2] == pytest.approx(p2.m_added_z, rel=1e-4)
...
def test_pitch_added_inertia_exceeds_planar_by_the_surge_offset(specs, p3):
    ...
    """
    p2 = mass_props(specs["V"])
    dz = p3.r_cg[2]                       # strips sit at z = 0 at Γ = 0
    assert p3.added[4, 4] - p2.m_added_x * dz ** 2 == pytest.approx(
        p2.i_added, rel=1e-4)
...
@needs_l1
def test_sideslip_added_mass_is_negligible(p3):
    """Edge-on a wing displaces almost nothing. Strip theory has no
    spanwise term at all, so this is structurally zero at Γ = 0 — fine
    for the modes, but it means sway inertia is bare airframe mass."""
    assert p3.added_sway < 1e-6 * p3.added_heave
